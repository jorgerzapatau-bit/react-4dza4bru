    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flexShrink: 0, padding: "18px 28px 0", borderBottom: "1px solid rgba(255,255,255,.05)", position: "sticky", top: 0, background: "#0f0f1a", zIndex: 50 }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", width: "100%" }}>
          ...
          {/* Tabs */}
          <div style={{ display: "flex", gap: 3, background: "rgba(255,255,255,.04)", borderRadius: 12, padding: 4 }}>
            {TABS.map((t, i) => (
              <button key={i} onClick={() => setTab(i)} style={{ flex: 1, padding: "8px 0", border: "none", borderRadius: 9, cursor: "pointer", background: tab === i ? "linear-gradient(135deg,#6c63ff,#e040fb)" : "transparent", color: tab === i ? "#fff" : "#4b4b6a", fontSize: 12, fontWeight: tab === i ? 700 : 500, fontFamily: "inherit", boxShadow: tab === i ? "0 2px 10px rgba(108,99,255,.35)" : "none", transition: "all .2s" }}>{t}</button>
            ))}
          </div>
              ...
              <div className="dash-col-grid">
                ...
                <div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="card" style={{ background: "rgba(255,255,255,.04)", borderRadius: 18, padding: 18, border: "1px solid rgba(255,255,255,.07)", marginBottom: 14 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: nuevosEsteMes.length > 0 ? 12 : 0 }}>
                          <div>
                            ...
                            <p style={{ color: "#4b4b6a", fontSize: 11 }}>{miembros.filter(m => m.estado === "Vencido").length} vencidos · {miembros.length} total</p>
                          </div>
                  ...
                  <div className="card" style={{ background: "rgba(255,255,255,.04)", borderRadius: 18, padding: "14px 18px", border: "1px solid rgba(255,255,255,.07)", marginBottom: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                ...
                <div>
                      ...
                      {membresiasPorVencer.map(m => {
                        ...
                        return (
                          <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(239,68,68,.1)", opacity: yaEnviado ? 0.5 : 1, transition: "opacity .3s" }}>
                            ...
                            <button onClick={() => marcarRecordatorio(m.id)} title={yaEnviado ? "Ya enviado hoy" : "Marcar como enviado"} style={{ width: 34, height: 34, border: `1px solid ${yaEnviado ? "rgba(74,222,128,.4)" : "rgba(255,255,255,.12)"}`, borderRadius: 10, background: yaEnviado ? "rgba(74,222,128,.12)" : "transparent", cursor: "pointer", color: yaEnviado ? "#4ade80" : "#4b4b6a", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all .2s" }}>
                              {yaEnviado ? "✓" : "💬"}
                            </button>
                          </div>
                        );
                  ...
                  {/* Últimos movimientos */}
                  <div className="card" style={{ background: "rgba(255,255,255,.04)", borderRadius: 18, padding: 18, border: "1px solid rgba(255,255,255,.07)" }}>
                    ...
                    {txsMes.length === 0 && <p style={{ color: "#4b4b6a", fontSize: 13, textAlign: "center", padding: "16px 0" }}>Sin movimientos este mes</p>}
