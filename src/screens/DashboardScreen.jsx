import { fmt, fmtDate, todayISO } from "../utils/dateUtils";
    ...
    <>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 24px 90px" }} className="gym-scroll-pad">
        ...
        {tab === 0 && <>
          ...
          <div className="card" style={{ background: "linear-gradient(135deg,#6c63ff 0%,#e040fb 100%)", borderRadius: 24, padding: 22, marginBottom: 14, boxShadow: "0 8px 32px rgba(108,99,255,.4)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6, position: "relative", zIndex: 1 }}>
              ...
              <button onClick={() => navMes(1)} style={{ background: isCurrentMonth ? "rgba(255,255,255,.05)" : "rgba(255,255,255,.15)", border: "none", borderRadius: 8, width: 28, height: 28, cursor: isCurrentMonth ? "default" : "pointer", color: isCurrentMonth ? "rgba(255,255,255,.2)" : "#fff", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
            </div>
          ...
          <div className="card" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
            {[
              { label: "Ingresos", val: totalIng, crec: crecIng, c: "#22d3ee", bg: "rgba(34,211,238,.08)", bc: "rgba(34,211,238,.2)" },
              { label: "Gastos", val: totalGas, crec: crecGas, c: "#f43f5e", bg: "rgba(244,63,94,.08)", bc: "rgba(244,63,94,.2)" }
            ].map((k, i) => (
          ...
          {/* Miembros activos + nuevos este mes */}
          {(() => {
              ...
              {membresiasPorVencer.map(m => {
                ...
                return (
                  <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(239,68,68,.1)", opacity: yaEnviado ? 0.5 : 1, transition: "opacity .3s" }}>
                    <div onClick={() => { setMensajesMiembro && setMensajesMiembro(null); setScreen("mensajes"); }}
        ...
        {tab === 1 && <>
          ...
          <button onClick={() => setModal("ingreso")} style={{ width: "100%", padding: "14px", border: "none", borderRadius: 14, cursor: "pointer", fontFamily: "inherit", fontSize: 14, fontWeight: 700, background: "linear-gradient(135deg,#22d3ee,#0ea5e9)", color: "#fff", marginBottom: 12 }}>+ Agregar ingreso</button>
          <div style={{ height: 12 }} />
        ...
        {tab === 2 && <>
          ...
          <button onClick={() => setModal("gasto")} style={{ width: "100%", padding: "14px", border: "none", borderRadius: 14, cursor: "pointer", fontFamily: "inherit", fontSize: 14, fontWeight: 700, background: "linear-gradient(135deg,#f43f5e,#e11d48)", color: "#fff", marginBottom: 12 }}>+ Agregar gasto</button>
          <div style={{ height: 12 }} />
