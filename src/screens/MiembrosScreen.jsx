    ...
    <>
      <div style={{ padding: "16px 24px 0", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          ...
          <h1 style={{ color: "var(--text-primary)", fontSize: 20, fontWeight: 700 }}>Miembros</h1>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          {(() => {
            ...
            return <button key={i} onClick={() => setFiltroEstado(s.val)} style={{ flex: 1, background: active ? `${s.bg}.18)` : "var(--bg-elevated)", border: active ? `1.5px solid ${s.bg}.35)` : `1.5px solid var(--border)`, borderRadius: 14, padding: "10px 8px", cursor: "pointer", fontFamily: "inherit", transition: "all .2s" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 4, alignItems: "center" }}>
          <div style={{ position: "relative", flex: 1 }}>
            ...
            <input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar..." style={{ width: "100%", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: 14, padding: "11px 12px 11px 36px", color: "var(--text-primary)", fontSize: 13, fontFamily: "inherit", outline: "none" }} />
          ...
          <button onClick={() => setViewMode(v => v === "lista" ? "grid" : "lista")}
            style={{ width: 44, height: 44, flexShrink: 0, border: "1px solid var(--border)", borderRadius: 14, background: "var(--bg-elevated)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "var(--text-primary)", transition: "all .2s" }}
      ...
      <div className="gym-scroll-pad" style={{ flex: 1, padding: "12px 24px 0" }}>
        ...
        {(() => {
          ...
          if (lista.length === 0) return <div style={{ textAlign: "center", padding: "40px 0" }}><p style={{ fontSize: 32, marginBottom: 12 }}>🔎</p><p style={{ color: "var(--text-tertiary)", fontSize: 14 }}>Sin resultados</p></div>;
          if (viewMode === "grid") {
            return (
                ...
                {lista.map(m => {
                  ...
                  return (
                    <div key={m.id} className="card rh" onClick={() => { setSelM(m); setModal("detalle"); }}
                      style={{ background: "var(--bg-card)", borderRadius: 18, padding: "14px 12px", border: "1px solid var(--border)", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, textAlign: "center" }}>
          ...
          return lista.map(m => {
            ...
            return (
              <div key={m.id} className="card rh" onClick={() => { setSelM(m); setModal("detalle"); }} style={{ background: "var(--bg-card)", borderRadius: 18, padding: "14px 16px", marginBottom: 10, border: showWA ? "1px solid rgba(37,211,102,.25)" : "1px solid var(--border)", cursor: "pointer" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <p style={{ color: "var(--text-tertiary)", fontSize: 11, marginTop: 2 }}>{mi.plan ? `Plan ${mi.plan}` : "Sin plan"} · 📱 {m.tel || "—"}</p>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    ...
                    {mi.estado === "Activo" ? <p style={{ color: "#22d3ee", fontFamily: "'DM Mono',monospace", fontSize: 13, fontWeight: 700, marginTop: 6 }}>{fmt(mi.monto)}</p> : <p style={{ color: "var(--text-tertiary)", fontSize: 11, fontWeight: 600, marginTop: 6 }}>—</p>}
                  </div>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--border)", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <p style={{ color: "var(--text-tertiary)", fontSize: 11 }}>Vence: {fmtDate(mi.vence) || "Por definir"}</p>
