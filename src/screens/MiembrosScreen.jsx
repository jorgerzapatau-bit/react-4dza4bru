        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", width: "100%" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
              <button
                ...
                style={{ background: "rgba(37,211,102,.15)", border: "1px solid var(--col-success-border)", borderRadius: 12, padding: "8px 12px", color: "var(--col-wa)", fontSize: 18, cursor: "pointer" }}
              >📢</button>
              <button
                onClick={() => {
                  setFM({
                ...
                style={{ background: "linear-gradient(135deg,var(--col-accent),var(--col-accent))", border: "none", borderRadius: 12, padding: "8px 14px", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {(() => {
              ...
              return [
                { label: "Activos", val: "Activo", count: miembros.filter(m => getMembershipInfo(m.id, txs, m).estado === "Activo" && m.estado !== "Pendiente").length, c: "var(--col-success)", bg: "rgba(74,222,128," },
                { label: "Vencidos", val: "Vencido", count: miembros.filter(m => { const s = getMembershipInfo(m.id, txs, m).estado; return (s === "Vencido" || s === "Sin membresía") && m.estado !== "Pendiente"; }).length, c: "var(--col-danger)", bg: "rgba(248,113,113," },
                { label: "Nuevos", val: "Nuevo", count: nuevosCount, c: "var(--col-info)", bg: "rgba(56,189,248," },
                ...(pendienteCount > 0 ? [{ label: "Pendientes", val: "Pendiente", count: pendienteCount, c: "var(--col-warning)", bg: "rgba(251,191,36," }] : []),
                ...(becasCount > 0 ? [{ label: "Becas", val: "Beca", count: becasCount, c: "var(--col-accent-text)", bg: "rgba(167,139,250," }] : []),
                { label: "Todos", val: "Todos", count: miembros.length, c: "var(--col-accent-text)", bg: "rgba(167,139,250," },
              ];
            })().map((s, i) => {
              ...
              return (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {lista.map(m => {
                    ...
                    const estadoColor = mi.estado === "Activo" ? "var(--col-success)" : mi.estado === "Sin membresía" ? "var(--text-secondary)" : "var(--col-danger)";
                    const estadoBg = mi.estado === "Activo" ? "var(--col-success-soft)" : mi.estado === "Sin membresía" ? "rgba(107,114,128,.15)" : "rgba(248,113,113,.15)";
                    return (
                      <div key={m.id} className="card rh" onClick={() => { setSelM(m); setModal("detalle"); }} style={{ background: "var(--bg-card)", borderRadius: 18, padding: "14px 12px", border: "1px solid var(--border)", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, textAlign: "center", position: "relative" }}>
                        ...
                        <div style={{ width: 58, height: 58, borderRadius: "50%", background: "linear-gradient(135deg,var(--col-accent)44,var(--col-accent)44)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--col-accent-text)", fontWeight: 700, fontSize: 22, overflow: "hidden", flexShrink: 0, boxShadow: `0 0 0 2px ${estadoColor}50` }}>
                ...
                <div key={m.id} className="card rh" onClick={() => { setSelM(m); setModal("detalle"); }} style={{ background: "var(--bg-card)", borderRadius: 18, padding: "14px 16px", marginBottom: 10, border: m.estado === "Pendiente" ? "1px solid rgba(251,191,36,.35)" : showWA ? "1px solid rgba(37,211,102,.25)" : "1px solid var(--border)", cursor: "pointer" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg,var(--col-accent)44,var(--col-accent)44)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--col-accent-text)", fontWeight: 700, fontSize: 18, overflow: "hidden", flexShrink: 0, boxShadow: m.estado === "Pendiente" ? "0 0 0 2px rgba(251,191,36,.5)" : "0 0 0 2px var(--col-accent-border)" }}>
                        {m.foto ? <img src={m.foto} alt={m.nombre} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : m.nombre.charAt(0)}
                      </div>
                      <div>
                        ...
                        <p style={{ color: "var(--text-tertiary)", fontSize: 11, marginTop: 2 }}>
                          ...
                          {m.tutor_nombre && <span style={{ color: "var(--col-warning)" }}> · 👨‍👧 Menor</span>}
                        </p>
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      {m.estado === "Pendiente" ? (
                        <span style={{ background: "rgba(251,191,36,.15)", color: "var(--col-warning)", borderRadius: 8, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>⏳ Pendiente</span>
                      ) : (
                        <span style={{ background: mi.estado === "Activo" ? "var(--col-success-soft)" : mi.estado === "Sin membresía" ? "rgba(107,114,128,.15)" : "rgba(248,113,113,.15)", color: mi.estado === "Activo" ? "var(--col-success)" : mi.estado === "Sin membresía" ? "var(--text-secondary)" : "var(--col-danger)", borderRadius: 8, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>{mi.estado}</span>
                      )}
                      {m.estado === "Pendiente"
                        ? <p style={{ color: "var(--col-warning)", fontSize: 11, fontWeight: 600, marginTop: 6 }}>Pago por confirmar</p>
                        : mi.estado === "Activo"
                        ? <p style={{ color: m.beca ? "var(--col-warning)" : "var(--col-info)", fontFamily: "'DM Mono',monospace", fontSize: 13, fontWeight: 700, marginTop: 6 }}>{m.beca ? "🎓 Beca" : fmt(mi.monto)}</p>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--border)", alignItems: "center" }}>
                    ...
                    {showWA && m.estado !== "Pendiente" && <span style={{ background: "rgba(37,211,102,.15)", color: "var(--col-wa)", borderRadius: 6, padding: "2px 7px", fontSize: 10, fontWeight: 700 }}>💬 {dias === 0 ? "hoy" : dias === 1 ? "mañana" : `${dias}d`}</span>}
                  </div>
                </div>
              );
            });
