...
function CalendarioEventos({ miembros, txs, getMembershipInfo, onGoToMember }) {
    ...
    miembros.forEach(m => {
      ...
      if (mem.vence && mem.estado !== "Sin membresía") {
        ...
        if (vp && vp.getMonth() === mesVer && vp.getFullYear() === anioVer) {
          agregarEvento(vp.getDate(), { tipo: "vence", nombre: m.nombre, foto: m.foto, miembro: m, color: mem.estado === "Vencido" ? "#f43f5e" : "#22d3ee", vencido: mem.estado === "Vencido" });
        }
      }
      // Nuevos miembros (fecha de primer ingreso en el mes)
      const txsM = txs.filter(t => t.miembro_id === m.id);
      if (txsM.length > 0) {
        const sorted = [...txsM].sort((a,b) => (a.fecha||"").localeCompare(b.fecha||""));
        const primera = sorted[0];
        if (primera.fecha) {
          const fp = new Date(primera.fecha + "T00:00:00");
          if (fp.getMonth() === mesVer && fp.getFullYear() === anioVer) {
            agregarEvento(fp.getDate(), { tipo: "nuevo", nombre: m.nombre, foto: m.foto, miembro: m, color: "#a78bfa" });
          }
        }
      }
    });
  ...
  const tipoLabel = { cumple: "🎂 Cumpleaños", vence: "⏰ Vence membresía", vencido: "🔴 Ya vencido", nuevo: "✨ Nuevo miembro" };
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
        <div style={{ background: "rgba(245,158,11,.08)", border: "1px solid rgba(245,158,11,.2)", borderRadius: 14, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <p style={{ color: "#f59e0b", fontSize: 20, fontWeight: 800, lineHeight: 1 }}>{totalCumples}</p>
            <p style={{ color: "#6b7280", fontSize: 10, marginTop: 2 }}>Cumpleaños</p>
          </div>
        </div>
        <div style={{ background: "rgba(167,139,250,.08)", border: "1px solid rgba(167,139,250,.2)", borderRadius: 14, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(167,139,250,.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>✨</div>
          <div>
            <p style={{ color: "#a78bfa", fontSize: 20, fontWeight: 800, lineHeight: 1 }}>{totalNuevos}</p>
            <p style={{ color: "#6b7280", fontSize: 10, marginTop: 2 }}>Nuevos</p>
          </div>
        </div>
        <div style={{ background: "rgba(34,211,238,.08)", border: "1px solid rgba(34,211,238,.2)", borderRadius: 14, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(34,211,238,.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>⏰</div>
          <div>
            <p style={{ color: "#22d3ee", fontSize: 20, fontWeight: 800, lineHeight: 1 }}>{totalVence}</p>
            <p style={{ color: "#6b7280", fontSize: 10, marginTop: 2 }}>Vencen este mes</p>
          </div>
        </div>
        <div style={{ background: "rgba(244,63,94,.08)", border: "1px solid rgba(244,63,94,.2)", borderRadius: 14, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(244,63,94,.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🔴</div>
          <div>
            <p style={{ color: "#f43f5e", fontSize: 20, fontWeight: 800, lineHeight: 1 }}>{totalVencidos}</p>
            <p style={{ color: "#6b7280", fontSize: 10, marginTop: 2 }}>Ya vencidos</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2, marginBottom: 14 }}>
        {celdas.map((dia, idx) => {
          ...
          return (
            <div key={idx} style={{
              borderRadius: 8, padding: "4px 2px", textAlign: "center", position: "relative", minHeight: 38,
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, marginBottom: 12 }}>
        {FILTROS.map(f => (
          <button key={f.key} onClick={() => setFiltro(f.key)} style={{
            border: "none", borderRadius: 20, padding: "5px 12px", cursor: "pointer", fontFamily: "inherit",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {listaEventos.map((ev, i) => (
            <div key={i} onClick={() => onGoToMember && ev.miembro && onGoToMember(ev.miembro)}
              ...
              <div style={{ width: 38, height: 38, borderRadius: "50%", background: `${ev.color}22`, display: "flex", alignItems: "center", justifyContent: "center", color: ev.color, fontWeight: 700, fontSize: 15, overflow: "hidden", flexShrink: 0, boxShadow: `0 0 0 2px ${ev.color}40` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: "#4b4b6a", fontSize: 10 }}>{ev.tipo === "cumple" ? "🎂 Cumpleaños" : ev.tipo === "nuevo" ? "✨ Nuevo miembro" : ev.vencido ? "🔴 Ya vencido" : "⏰ Vence membresía"}</p>
              </div>
              <span style={{ color: ev.color, fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{String(ev.dia).padStart(2,"0")} {MESES_SHORT[mesVer]}</span>
            </div>
          ))}
        </div>
      )}
          ...
          <div style={{ padding: "16px 24px 0", flexShrink: 0 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <button onClick={() => setScreen("estadisticas")} title="Estadísticas" style={{ width: 40, height: 40, borderRadius: 14, border: "none", cursor: "pointer", background: "rgba(255,255,255,.07)", fontSize: 18 }}>📊</button>
